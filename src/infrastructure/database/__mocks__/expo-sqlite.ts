    ...
    if (sql.includes('WHERE chain = ?')) {
      ...
      for (const condition of conditions) {
        if (condition.includes('protein_g >=')) {
          ...
          results = results.filter(item => item.energy_kcal !== null && item.energy_kcal >= minEnergy);
        } else if (condition.includes('energy_kcal <=')) {
          const maxEnergy = params[paramIndex++];
          results = results.filter(item => item.energy_kcal !== null && item.energy_kcal <= maxEnergy);
  ...
  async getFirstAsync(sql: string, params: any[] = []): Promise<any> {
    ...
    if (sql.includes('SELECT MAX(last_seen_at)')) {
      ...
      const maxDate = items.reduce((max, item) => 
        item.last_seen_at > max ? item.last_seen_at : max, 
